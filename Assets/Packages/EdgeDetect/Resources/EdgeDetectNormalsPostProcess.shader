...
Shader "Hidden/EdgeDetect-PostProcess"
{
    ...
    half4 output(half4 src, half4 col) {
      return lerp(src, col, _T);
    }
		...
		float4 FragRobert(Varyings i) : SV_Target
		{
			...
			return output(color, lerp(_EdgeColor, _BgColor, edge));
		}
      ...
      Sobel = 1.0 - pow(saturate(Sobel), _Exponent);
			return output(color, lerp(_EdgeColor, _BgColor, Sobel));
		}
		...
		float4 FragLum(VaryingsLum i) : SV_Target
		{
			...
			return output(color, lerp(_EdgeColor, _BgColor, saturate(len)));
		}
